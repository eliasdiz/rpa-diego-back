...
// Hace exactamente lo mismo que automatizacion.js pero con Stagehand
...
console.log('🚀 Iniciando script de automatización con Stagehand...');
...
export async function automatizarConStagehand() {
    console.log(`🚀 [${horaActual()}] Iniciando automatización con Stagehand...`);
    ...
    try {
        ...
        console.log(`🔧 [${horaActual()}] Inicializando Stagehand (conectando a navegador existente)...`);
        stagehand = new Stagehand({
            ...
            localBrowserLaunchOptions: {
                headless: false,
                // No especificar userDataDir ni remote-debugging-port para evitar conflictos
                args: [
                    '--no-first-run',
        ...
        console.log(`✅ [${horaActual()}] Stagehand iniciado correctamente`);
        
        const page = stagehand.page;
        console.log(`📄 [${horaActual()}] Página obtenida de Stagehand`);
        ...
        let intentos = 0;
        const maxIntentos = 10; // Limitar para pruebas
        ...
        while (prospectosCambiados < cantidad && intentos < maxIntentos) {
            ...
            try {
                ...
                // Proceso de cambio de propietario usando Stagehand
                try {
                    console.log(`🔍 [${horaActual()}] Buscando campo de usuario...`);
                    
                    // Usar Stagehand para encontrar el campo de usuario
                    await stagehand.act({ action: "click", text: "diego.alvarez@asesorsura.com" });
                    await page.waitForTimeout(1000);

                    console.log(`💾 [${horaActual()}] Guardando cambios...`);
                    
                    // Buscar y hacer clic en el botón de guardar
                    await stagehand.act({ action: "click", text: "Guardar" });
                    await page.waitForTimeout(3000);
...
export async function ejecutar() {
    try {
        ...
        const resultado = await automatizarConStagehand();
...
if (import.meta.url === `file://${process.argv[1]}`) {
    console.log('🚀 Script ejecutado directamente, iniciando automatización...');
